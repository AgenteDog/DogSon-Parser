...
extention_choose = input('Please input the file extention: ')
...
if extention_choose == 'map':
    ...
    print(Fore.RED + 'The extension has to be "map" or "vmf"')
    os.system("pause")
    sys.exit()
...
if os.path.isfile(mapfile) == True and missing_file == False:
    ...
    for entity in entities:
        ...
        value['RotX'] = f"{float(xr):.4f}"
        value['RotY'] = f"{float(yr):.4f}"
        value['RotZ'] = f"{float(zr):.4f}"
...
if mapents == True:
    if os.path.isfile(mapentsfile) == True and missing_file == False:
        ...
        for entity_mapents in entities_mapents:
            ...
            value_mapents['RotX'] = f"{float(xr):.4f}"
            value_mapents['RotY'] = f"{float(yr):.4f}"
            value_mapents['RotZ'] = f"{float(zr):.4f}"
